...
def bot_echo_polling(bot_url: str, polling_interval=1):
    ...
    is_command, last_update_id, last_message_id, chat_id, text = parse_message(update)
...
def bot_set_commands(bot_url: str):
    ...
    commands = [
        ...
        {"command": "time", "description": "send current time"},
        {"command": "help", "description": "print help message to user"},
        {"command": "getlvl", "description": "get current level of user"},
        {"command": "setlvl", "description": "set level to user"},
    ]
                ...
                if is_command:
                    ...
                    if not level:
                        if not remote and last_message_text != text:
                            last_message_text = text
                            user_lvl = [u.level for u in users if u.id == chat_id][0]
                            result_msg = get_sentences_from_local(text, user_lvl)
                            print(f"User level is {user_lvl}")
                            send_message(bot_url, chat_id, result_msg)
                        if remote and last_message_text != text:
                            last_message_text = text
                            user_lvl = [u.level for u in users if u.id == chat_id][0]
                            result_msg = get_sentences_from_remote(text, user_lvl, 10)
                            print(f"User level is {user_lvl}")
                            send_message(bot_url, chat_id, result_msg)
